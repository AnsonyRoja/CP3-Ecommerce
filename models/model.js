...
module.exports = {
  ...
  addCategory: function (category) {
    ...
    const exist = categories.find(cat => cat === category);

    if (exist) throw Error("La categoría ya existe");
  ...
  addProduct: function (name, brand, category, stock) {
    ...
    if (!exist) throw Error('La categoría ingresada no existe');

    const categoryId = categories.findIndex(cat => cat === category) + 1;

    let avaible = false;

    if (stock > 0) avaible = true
    else
      avaible = false;
  ...
  listProducts: function (category, fullName) {
    ...
    if (category) {
      ...
      if (fullName) {
        return filteredProducts.map(product => `${product.brand} ${product.name}`);
      }
  ...
  addReview: function (name, stars, text, user) {
    ...
    // console.log("name", name, "star", stars, "text", text, "user", user);

    if (stars === undefined || text === undefined || user === undefined) throw Error("Faltan parámetros");

    const encontrarProduct = products.find(pro => pro.name === name);
    // console.log(encontrarProduct);



    if (isNaN(stars) || stars < 1 || stars > 5) throw Error("Puntaje inválido")

    if (encontrarProduct === undefined) throw Error('Producto no encontrado');

    const review = { stars, text, user }

    encontrarProduct.reviews.push(review);

    const totalStars = encontrarProduct.reviews.reduce((sum, review) => sum + review.stars, 0);
    encontrarProduct.rating = totalStars / encontrarProduct.reviews.length;
  ...
  getRating: function (name) {
    ...
    if (name === undefined) {
      ...
      return products.slice(0, 5).map(pro => pro.name);
    };


    const buscarProductName = products.find(prod => prod.name === name);

    if (buscarProductName === undefined) throw Error('Producto no encontrado');



    return buscarProductName.rating;
